...
namespace drawing {
    ...
    void readVideoFromWebcam(const int& channel) {
        ...
        if (vid.open(channel)) {
            ...
            {
                vid.retrieve(frame); // retrieve = videonun çıkış dizisini, Mat tipindeki değere aktarma.
    ...
    void createSimpleImage(const int& height, const int& width, const int& blue, const int& green, const int& red) {
        cv::Mat img(height, width, CV_8UC3); // CV_8UC3  = blue, green, red; 3 kanallı renk uzayı;
        img = cv::Scalar(blue, green, red); // Scalar, resime renk vermek için kullanılır. blue-green-red -> min=0 max=255
